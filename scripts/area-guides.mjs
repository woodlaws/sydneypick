...
export function renderAreaGuide(g) {
  ...
  <section class="editorial-section tint"><div class="wrap"><header class="editorial-head"><div><p class="eyebrow">PRACTICAL GUIDE</p><h2>현장에서 필요한 선택 기준</h2></div><p>가격·운영시간·교통시간은 확정값으로 싣지 않고 당일 공식 안내를 기준으로 합니다.</p></header><div class="info-columns"><article class="info-panel"><h3>교통과 이동 팁</h3><p>${g.transport}</p></article><article class="info-panel"><h3>사진 촬영 포인트</h3><p>${g.photo}</p></article><article class="info-panel"><h3>식사·카페 선택 기준</h3><p>${g.food}</p><a href="${g.slug === "the-rocks" ? "/food/by-area" : g.slug === "bondi-beach" || g.slug === "manly" ? "/food/brunch-cafes" : "/food/by-area"}">관련 맛집·카페 가이드 →</a></article><article class="info-panel"><h3>우천 시 대안</h3><p>${g.rain}</p></article><article class="info-panel"><h3>방문 전 확인사항</h3><ul>${list(g.checks)}</ul></article><article class="info-panel"><h3>관련 5박 6일 일정</h3><p>${g.day}</p><a href="/itineraries/sydney-5n6d#${g.dayAnchor}">일정에서 보기 →</a></article></div></div></section>
