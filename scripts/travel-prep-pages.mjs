...
const hero = (page, parent=false) => `<main id="content"><section class="prep-hero"><img src="${page.image}" alt="${page.alt}"><div class="wrap prep-hero-content"><nav class="breadcrumb" aria-label="현재 위치"><a href="/">홈</a><span>›</span>${parent?'<a href="/travel-prep">여행 준비</a><span>›</span>':''}<span>${parent?page.h1:"여행 준비"}</span></nav><p class="eyebrow">${page.eyebrow}</p><h1>${page.h1}</h1><p class="lead">${page.lead}</p></div><p class="credit">Photo: Unsplash</p></section>`;
