...
const assertions = [
  ...
  [html.includes('class="home-hero-title"><span>처음 가는 시드니,</span><span>실패 없는 선택</span>') && html.includes('class="home-section-title"') && !html.includes("KOREAN TRAVELLER'S GUIDE TO SYDNEY"), "dedicated simplified home hero structure"],
  [css.includes('.home-hero-title{max-width:850px;font-size:clamp(56px,4.4vw,72px)') && css.includes('.home-section-title{max-width:760px') && !css.includes('h1{margin:0;font-size'), "home title sizing is scoped and global H1 sizing removed"],
  ...
  [magazineHub.includes('class="magazineHeroInner magazineContainer"') && magazineHub.includes('class="magazineTitle"') && magazineHub.includes('class="magazineDescription"') && magazineHub.includes('class="magazineCategoryScroll"') && magazineHub.includes('class="mag-category-nav-inner categoryNavigationInner magazineCategoryNavInner"'), "shared magazine hub container and category navigation structure"],
  [magazineCategoriesHtml.every((page)=>page.includes('class="categoryHeroInner magazineContainer"')&&page.includes('class="magazineCategoryHeroContent"')&&page.includes('class="breadcrumb magazineBreadcrumb"')&&page.includes('class="magazineCategoryContent magazineContainer"')&&page.includes('class="magazineCategoryScroll"')&&page.includes('class="mag-category-nav-inner categoryNavigationInner magazineCategoryNavInner"')), "shared centered magazine category layout"],
  [magazineCategoriesHtml.every((page)=>page.includes('class="magazine-page magazineCategoryPage"'))&&magazineCss.includes('.magazineCategoryGrid[data-count="1"]{grid-template-columns:minmax(0,420px)')&&magazineCss.includes('.magazineCategoryGrid[data-count="2"]{max-width:860px'), "category grids respond to post count"],
  ...
  [magazineCategoriesHtml.length === 9 && magazineCategoriesHtml.every((page) => (page.match(/<h1(?:\s|>)/g) || []).length === 1), "category routes including empty states"],
